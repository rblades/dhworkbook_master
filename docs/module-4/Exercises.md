...
So, place your cursor in cell B1. On a Mac, hit `shift+cmnd+downarrow`. On a Windows machine, hit `shift+ctrl+downarrow`. Then on Mac `shift+cmnd+rightarrow`, on Windows `shift+ctrl+rightarrow`. Then copy all of that data (`cmnd+c` or `ctrl+c`). Then, under 'Edit' select 'paste special' -> 'paste VALUES only'.
